...
1. **Corpus** — Drive `ClinicOps-Autonomy/06_TERMBASE/corpus/`: bilingual pairs placed there, paired by filename (`<name>_EN.*` with `<name>_DA.*`). Read with `read_file_content` (plain-text files come back with markdown escaping such as `\#` and `\_`; strip the backslashes before quoting). A file whose first line starts with `SOURCE:` or `KILDE:` and names a public legal source (EUR-Lex, legislation.gov.uk, retsinformation.dk) is PUBLIC-source: its rows may go into the Sunday public PR. Every other corpus file is PRIVATE: its rows go to Drive and the project only, never to GitHub. Work through a large pair over several nights (about 20–40 aligned pairs per night); record in `TERMBASE_PROGRESS.md` which file and which numbered item you reached.
2. **Public aligned legal texts** — Regulation (EU) 2017/745 (MDR) and 2017/746 (IVDR), EN and DA, via WebFetch on EUR-Lex (`https://eur-lex.europa.eu/legal-content/EN/TXT/HTML/?uri=CELEX:32017R0745` and the `DA` equivalent; IVDR is CELEX:32017R0746). Work through: Article 2 definitions → Annex I general safety and performance requirements headings → Annex XV clinical investigation terms → Chapter VII vigilance terms → Annex II/III technical documentation headings. If EUR-Lex truncates, use `legislation.gov.uk/eur/2017/745/…/adopted` for EN and re-fetch EUR-Lex DA by article anchor. If WebFetch is refused in this session (PROVENANCE_REQUIRED), do not retry more than once per URL; note the block in the log and the NEEDS_ALI item only if it is not already listed, and continue with source 1.
