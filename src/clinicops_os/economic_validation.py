...
# Fail-closed provenance: a ledger must explicitly declare itself "real" before
# its events can produce economic proof. Synthetic/example/test ledgers (and any
# ledger that does not declare a class) exercise the control machinery only.
LEDGER_CLASSES = ("synthetic", "real")
EVENT_KINDS = {
...
class EconomicLedger:
    ...
    def from_dict(cls, row: dict[str, object]) -> EconomicLedger:
        ...
        if not isinstance(thresholds, dict):
            raise TypeError("thresholds must be a JSON object")
        ledger_class = _optional_string(row, "ledger_class") or "synthetic"
        if ledger_class not in LEDGER_CLASSES:
            raise ValueError(
                f"unknown ledger_class '{ledger_class}'; expected one of: "
                + ", ".join(LEDGER_CLASSES)
            )
        return cls(
...
class EconomicEvaluation:
    ...
    control_pass: bool
    exp001_pass: bool
    simulated_initial_economic_proof: bool
    simulated_repeatability_proof: bool
...
def evaluate_economic_ledger(ledger: EconomicLedger) -> EconomicEvaluation:
    ...
    simulated_repeatability_proof = simulated_initial_proof and (
        len(paid_buyers) >= thresholds.min_paid_buyers_for_repeatability
        or len(expansion_buyers) >= 1
    )

    # Fail closed: only a ledger explicitly declared "real" can emit economic
    # proof. Synthetic/example ledgers demonstrate the control machinery only.
    is_real = ledger.ledger_class == "real"
    if not is_real and simulated_initial_proof:
        findings.append(
            EconomicFinding(
                "-",
                "info",
                "ledger_class is not 'real': proof thresholds were met mechanically, "
                "but initial_economic_proof/repeatability_proof remain false (fail closed)",
            )
        )
    ...
    return EconomicEvaluation(
        ...
        simulated_initial_economic_proof=simulated_initial_proof,
        simulated_repeatability_proof=simulated_repeatability_proof,
        initial_economic_proof=simulated_initial_proof and is_real,
        repeatability_proof=simulated_repeatability_proof and is_real,
...
def render_evaluation(evaluation: EconomicEvaluation) -> str:
    payload = {
        ...
        "ledger_class": evaluation.ledger_class,
        "control_pass": evaluation.control_pass,
        "exp001_pass": evaluation.exp001_pass,
        "simulated_initial_economic_proof": evaluation.simulated_initial_economic_proof,
        "simulated_repeatability_proof": evaluation.simulated_repeatability_proof,
